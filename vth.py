...
# Tạo lock để đồng bộ in ra console
_console_lock = threading.Lock()
_console_busy = False
_in_menu = False  # Biến kiểm tra đang ở menu hay không
...
def safe_console_print(*args, **kwargs):
    ...
    with _console_lock:
        ...
        _console_busy = True
        try:
            # Kiểm tra nếu có spinner trong kwargs, bỏ qua vì không hỗ trợ
            if 'spinner' in kwargs:
                kwargs.pop('spinner', None)
            console.print(*args, **kwargs)
        except Exception as e:
            # Fallback nếu Rich bị lỗi
            try:
...
def safe_console_status(message, spinner="dots"):
    """Hiển thị status với context manager an toàn"""
...
# Thêm handler để log ra file thay vì console
...
def on_open(ws):
    ...
    # Không in ra console để tránh xung đột với menu
...
def start_game_flow():
    ...
    _in_menu = False  # Đang ở chế độ chơi
    ...
    # Sử dụng console.status thay vì safe_console_print với spinner
...
def main_vth():
    global _in_menu
    console.clear()
    welcome = Panel(Align.center(Text.assemble((f"{ICONS['crown']} ", f"bold {HTOOL_COLORS['gold']}"), ("WELCOME TO ", "bold white"), ("HTOOL VIP PREMIUM", f"bold {HTOOL_COLORS['neon_blue']}"), (f" {ICONS['crown']}", f"bold {HTOOL_COLORS['gold']}"))), border_style=HTOOL_COLORS["gold"], box=box.DOUBLE)
